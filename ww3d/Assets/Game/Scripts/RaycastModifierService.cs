...
///     For navmesh/recast graphs the <see cref="FunnelModifierService" /> is a much better and faster alternative.
...
/// </summary>
[LevelScope]
public class RaycastModifierService : ISelfRegisterable {
    ...
    private List<Vector3> ApplyDP(Path p, List<Vector3> points) {
        ...
        for (var i = 0; i < DPParents.Length; i++) {
        ...
        if (useRaycasting) {
            ...
            if (use2DPhysics) {
                if (thickRaycast && thickRaycastRadius > 0 &&
                    Physics2D.CircleCast(v1 + raycastOffset, thickRaycastRadius, v2 - v1, (v2 - v1).magnitude, mask)) {
                    return false;
                }
                ...
                if (thickRaycast && thickRaycastRadius > 0 &&
                    Physics.SphereCast(new Ray(v1 + raycastOffset, v2 - v1), thickRaycastRadius, (v2 - v1).magnitude, mask)) {
                    return false;
                }
